...
export const uploadFile = async (uridata, extention) => {
    let uri = ''
    try {

        console.log("fjkdsbfjdbfhbfjdsfbsdjfbh")
        let pathd = RNFS.DocumentDirectoryPath 
           await CompressImage.createCompressedImage(uridata,pathd).then((response) => {
            uri  =response.uri
            console.log("urisdfdsfdsfds",uri)
